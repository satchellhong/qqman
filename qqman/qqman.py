...
def manhattan(
    ...
    **kwargs,
    ...
    if not (isinstance(cmap_var, (numbers.Number, list))):
    ...
    chr_len = [
        len(df_assoc[df_assoc[col_chr] == cChr]) for cChr in df_assoc[col_chr].unique()
    ]
    ...
    for cChr in df_assoc[col_chr].unique():
        last_ind = 0 if len(list_ind) == 0 else (list_ind[-1] + 1) + (gap * weight_gap)
        list_ind += [
...
def qqplot(
    ...
    **kwargs,
    ...
    if isinstance(assoc, str):
        ...
        p_vals = [ele for ele in assoc if 0 < ele < 1]
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.error(f"qqman.py failure on arguments: {args}")
